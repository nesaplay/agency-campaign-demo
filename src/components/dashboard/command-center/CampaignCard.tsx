...
const CampaignCard: React.FC<CampaignCardProps> = ({ campaign }) => {
  ...
  return (
    <Card
      className={cn(
        "card-component card-campaign interactive w-[300px] min-w-[300px] cursor-pointer transition-all duration-200",
        expanded ? "scale-[1.02] shadow-elevated" : "",
        campaign.needsAttention ? `${colors.border} ring-1 ring-warning/30` : ""
      )}
      onClick={handleClick}
      style={{
        padding: 0, // override Card default if needed, using 24px on inner sections
        borderRadius: 8
      }}
    >
      <CardContent className="!p-0">
        <div className="relative">
          {campaign.needsAttention && (
            <div className="absolute -top-2 -right-2 bg-warning text-white p-1 rounded-full z-10 shadow-md">
          ...
          <div className="px-6 py-6 space-y-6">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="text-xs text-right mb-1">
                {campaign.primaryDifference > 0 ? (
                  <span className="data-text text-success">+{campaign.primaryDifference}%</span>
                ) : (
                  <span className="data-text text-error">{campaign.primaryDifference}%</span>
                )}
              </div>
            </div>
